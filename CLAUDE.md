...
- **Playlist sync: the phone's playlists are a mirror keyed on the desktop's ids.** Internal (user-created) playlists sync both ways; Bandcamp's own playlists stay read-only. The desktop is authoritative, and `create-playlist`/`add-track-to-playlist`/`reorder-playlist-tracks` all accept **caller-supplied ids** so an edit made offline replays as the *same* playlist on both devices. Remote+connected writes are unchanged (straight `send()` to the PC, pure remote control); Standalone **and remote-with-the-socket-down** write locally *and* append the op to the `playlist_ops` outbox. Reads come from the mirror in **both** modes, so the list renders offline and instantly — `setMode('remote')` no longer blanks `playlists`. Conflict resolution is **last-op-wins with no tombstones and no timestamps**: a phone-side delete that flushes after a desktop edit destroys that edit. Single-user, single-account — that trade is deliberate, don't build conflict UI.
- **Sync direction is a desktop setting (`AppSettings.playlistSyncMode`), not a mobile one.** `two-way` (default) / `desktop-to-mobile` / `mobile-to-desktop` / `disabled` gate the two halves of `PlaylistSyncService.sync()`: push = flush + bootstrap, pull = the mirror refresh. The phone has only an on/off switch (`playlistSyncEnabled`), which can *reduce* syncing, never enable a direction. The mode is **fetched per `sync()`** (`get-playlist-sync-mode` → `playlist-sync-mode`, outside `SERIALIZED_MESSAGE_TYPES`) rather than pushed on change — one mechanism, never stale; a timeout falls back to the value persisted in the mobile `settings` table. The desktop **re-enforces** it in `handleMessage`, keyed on `deviceInfo` (only `identify` sets it, and `client.js` never sends it) so the web remote keeps full editing while a #110-era phone — which pushes unconditionally, and unlike the desktop does not auto-update — is silently dropped. `canEditPlaylists()`/`canPushPlaylists()` in the mobile store gate all ten mutating actions plus the single `queuePlaylistOp` enqueue chokepoint; a mode that cannot push also **drains the outbox** so nothing replays weeks later.
- **`playlists.from_desktop` is what makes delete-on-absence safe**, not the bootstrap flag. Only playlists that actually arrived from the desktop (`importPlaylist(p, true)`) can be removed by a pull, so a phone-created or file-imported playlist is stranded but never destroyed — which is what makes the one-directional modes non-destructive. The old `allowDeletes && outbox empty` gate still applies on top. The bootstrap skips `from_desktop = 1` rows (a mirrored library would otherwise seed a pointless multi-MB flush) and is *deferred*, not lost, when the current mode cannot push. Identity is still the id, never the name: a "Chill" on each device before sync yields two "Chill"s, a duplicate, never a merge.
- **An outbox row *is* the wire message.** `playlist_ops` stores `{type, payload}` verbatim and `flush()` is `webSocketService.send(op.type, op.payload)`, so adding a syncable operation means enqueuing the same message the connected path already sends — no separate op vocabulary to keep in step. `PlaylistSyncService.sync()` is single-flight: flush → pull → re-loop if the outbox refilled (capped at 3 cycles). It deletes only ops **up to the last id actually sent** (`lastSentId`, not the batch max), so a socket that dies mid-flush leaves the rest queued, and an op appended *during* the flush survives to the next cycle. There are no acks, so an op the desktop rejects is dropped, not retried; ops naming a playlist absent from the post-flush snapshot are counted and surfaced as one toast.
- **The pull diffs on `(id, updatedAt, trackCount)`, never on a full snapshot.** `playlists-data` arrives with `tracks: []` (a few KB) and only changed ids are hydrated via `get-playlist-for-export`; shipping whole playlists per change would be 2–6 MB of JSON parsed on the RN JS thread. `trackCount` is not redundant — it catches two edits inside the same ISO millisecond. This is also why **`reorderPlaylistTracks` must bump `playlists.updated_at`** (it didn't, and desktop reorders were invisible to the phone forever); the bump lives in the shared `writePositions` helper so `setPlaylistTrackOrder` can't forget it.
- **Deleting on "absent from the snapshot" needs two gates, and one of them is per-cycle.** A local playlist missing from the host is only safely deletable when the outbox is empty **and** the H4 bootstrap ran on an *earlier* sync — `allowDeletes` is therefore captured *before* `bootstrapIfNeeded()`. Capturing it after looks equivalent and is not: the bootstrap's own creates are fire-and-forget, so if one doesn't land, the very first sync would read the playlist as "deleted on the desktop" and destroy it. The bootstrap (one-time, behind an AsyncStorage flag) seeds create + bulk add-track ops for every pre-existing local playlist, keeping their current mobile ids — it never destroys, and its worst case is a duplicate.
- **`setPlaylistTrackOrder(playlistId, orderedEntryIds)` contract**: entries named in the list are placed in that order first; entries the DB holds but the list omits **keep their relative order and are appended after**; ids not in the DB are ignored. This is what stops a stale reorder from a second client truncating a playlist, and it is the one non-obvious invariant in the feature. An offline reorder ships absolute entry ids, never the `from`/`to` index pair — by replay time the host's indices mean something else. `client.js` still sends the index pair, so both forms are supported.
- **Replay requires `ON CONFLICT(id) DO NOTHING` on both inserts, on both platforms.** Caller-supplied ids alone are not enough: a plain `INSERT` throws `SQLITE_CONSTRAINT_PRIMARYKEY` and aborts the whole flush. Playlist entry ids are now plain `uuidv4()` — the old `` `${playlistId}-${track.id}-${Date.now()}` `` form collided when the same track was added twice inside one millisecond.
- **`import-playlist` deliberately ignores ids in its payload.** An exported file carries the ids it was exported with, so honouring them would make importing your own export silently merge into the original instead of copying it. The sync flush never routes through `import-playlist`; it decomposes into `create-playlist` + bulk `add-track-to-playlist` ops.
- **The host's `ws.on('message')` is an un-awaited async handler, so send order is not processing order.** `add-track-to-playlist` can hit the network (`resolveTrack` → `getAlbumDetails`), which means a flushed batch interleaves at every `await` and lands out of order. Two mitigations, both needed: mobile decomposes album/radio/import adds into **already-resolved** bulk ops so the host never scrapes on the flush path, and the playlist-mutating cases plus `get-playlists`/`get-playlist-for-export` run behind a **per-connection** promise chain (`SERIALIZED_MESSAGE_TYPES`). Never chain globally — `pause` must not queue behind a 5s album scrape.
- **Never flush in response to `playlists-data`.** Every host mutation emits `playlists-changed` and broadcasts a snapshot to *every* client, so flushing on it amplifies. Flush only on socket open and on a local mutation while connected; treat inbound `playlists-data` as a **pull hint** that `sync()` drops outright while a cycle is in flight (that cycle ends with a pull anyway). `handlePlaylistsChanged` is separately trailing-debounced ~150 ms, which also stops a 20-track add spraying 20 broadcasts at the web remote.
- **Every sync trigger is keyed on the socket, never on the mode.** All three — socket open (`connection-status`), the inbound `playlists-data` pull hint, and `refreshPlaylists()` — used to be inside `mode === 'remote'` guards, which silently limited the feature to Remote mode: a Standalone phone keeps a background socket and pushes its own edits (`queuePlaylistOp` only checks `isConnected()`), but nothing ever triggered a *pull*, so desktop-side changes were invisible until the user happened to edit a playlist on the phone. `sync()` already no-ops when the socket is down, so `webSocketService.isConnected()` is the only gate any trigger needs. Enabling the pull in Standalone is not destructive: delete-on-absence is still limited to `from_desktop = 1` rows. `store/index.test.ts` locks this with "syncs in standalone mode on socket open and on the pull hint" — restoring either mode guard fails exactly that test (mutation-verified).
- **Mobile playlist deletes must delete `playlist_tracks` explicitly.** `PRAGMA foreign_keys` was never enabled on the mobile connection, so `ON DELETE CASCADE` was inert and every `deletePlaylist` left orphan rows — which then merge into the pulled tracks if that id later returns from the desktop. Both fixes are needed: the pragma is now in `setupTables`, *and* the delete is explicit, because the pragma is per-connection and easy to lose. A one-time `DELETE FROM playlist_tracks WHERE playlist_id NOT IN (SELECT id FROM playlists)` clears the orphans already out there.
- **Mobile `removeTrackFromPlaylist` removes exactly one entry, matching the desktop.** Its old `(id = ? OR json_extract(track_data, "$.id") = ?)` clause removed *every* copy of a duplicated track; replaying that against the desktop's single-entry delete left local 0 / host 2 and the pull re-added them — permanent flapping. The UI already passes `playlistEntryId || item.id`.
...
- **A `jest.mock()` factory is hoisted above the `const` it closes over** — so `jest.mock('../services/MobileDatabase', () => ({ mobileDatabase: mockDb }))` hands the module `undefined`, and every call on it dies with "Cannot read properties of undefined". Babel rewrites the `const` to a `var`, so there is no TDZ error to point at the cause. Return a **getter** instead (`get mobileDatabase() { return mockDb; }`), which resolves at access time. `PlaylistSyncService.test.ts` does this for both of its module mocks.
- **`jest.clearAllMocks()` does NOT undo `mockImplementation`** (only `mockReset` does, and it would wipe the factory's own implementation too). A test that swaps in a stub implementation therefore leaks into *every later test in the file*, and the symptom is unrelated tests failing further down. Either use `mockImplementationOnce`, or re-apply the defaults in `beforeEach` — `PlaylistSyncService.test.ts` keeps its default impls as named `mockDefault*` consts and re-applies them, which is what makes the fake-desktop mock reusable across 14 tests.
- **A fake that ignores the messages it receives tests less than it appears to.** The playlist-sync round trip is only meaningful because the mocked `webSocketService` **applies** the playlist messages to an in-memory host map, so a flush is observable in the pull that follows. With a fake that just recorded sends and replayed a hand-written snapshot, three tests passed for the wrong reason and one hid a real bug (the first sync deleting a playlist whose create had not landed).
