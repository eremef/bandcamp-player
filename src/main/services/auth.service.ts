...
import type { AppSettings, AuthState, BandcampUser } from "../../shared/types";
...
export class AuthService {
   ...
   */
  private async fetchProfileFromId(
    fanId: string,
  ): Promise<BandcampUser | null> {
    // Skip network request when offline mode is active — return minimal fallback immediately
    const settings = this.getSettings?.();
    if (settings?.offlineMode) {
      console.log(
        "[AuthService] Offline mode: skipping Bandcamp profile fetch, using fallback",
      );
      return {
        id: fanId,
        ...
        request.on("response", (response: any) => {
          ...
          response.on("end", () => {
            try {
              ...
              console.log(
                "User info not found in menubar API response keys:",
                Object.keys(data),
              );
