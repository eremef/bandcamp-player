...
- **NPM Install Collision Prevention**: Before running `npm install` autonomously in a background task, always check the `ADDITIONAL_METADATA` running terminal commands to verify that the user is not already running `npm install`. This prevents file lock collisions (`TAR_ENTRY_ERROR ENOENT` and `EPERM`) caused by concurrent processes mutating `node_modules` simultaneously.
